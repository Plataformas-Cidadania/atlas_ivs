<?php
    ...
    class PublicMethods{
        ...
        public static $ArrayPadding3ZerosDireita = array(196,197,198,199,1,2,3,4);
        ...
        */
        ...
        public static function nameJson($lugar, $indicador){
            ...
            foreach($lugar as $key=>$val){
                ...
                $muns = explode(",",$val['mun']);
                sort($muns);
                $strm = implode(',', $muns);

                $ests = explode(",",$val['est']);
                sort($ests);
                $stre = implode(',', $ests);
                ...
                $udhs = explode(",",$val['udh']);
                sort($udhs);
                $stru = implode(',', $udhs);

                $sv[] = $val["e"].'|'.$str.'|'.$strm.'|'.$stre.'|'.$strr.'|'.$stru;
            }
            $inc = explode(',', $indicador);
            sort($inc);
            $resInc = implode(',',$inc);
            $resSv = implode(',', $sv);
            //$r = "j".md5($resInc."||".$resSv."||".$ord);
